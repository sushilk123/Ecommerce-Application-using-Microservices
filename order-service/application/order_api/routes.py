...
def order_add_item():
    api_key = request.headers.get('Authorization')  #user api-key
    ...
    #check if the user id has any open orders:
    ...
    if known_order is None:
        ...
        #if the item with existing product id exists, then increment the quantity
        for item in known_order.items:
            if item.product_id == p_id:
                found = True
                item.quantity += qty

        #if item is not found create a new order item
        if found is False:
            order_item = OrderItem(p_id, qty)
            known_order.items.append(order_item)
...
def checkout():
    ...
    order_model.is_open = 0  #if there is an open order, set that order to false
